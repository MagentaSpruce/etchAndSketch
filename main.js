...
let gridSize = 14;
const gridContainer = document.querySelector('.gridBox__container');
let bgColor = '#ddffbc';
...
function createGrid(){
    ...
    const lastItems = [gridItems].slice(-`${gridSize}`);
...
colorPicker.addEventListener('input', (e) => {
    ...
    if(grab) {
        grab = false;
        dropper.classList.remove('btnOn');
    }
});
...
for(let i = 0; i < buttons.length; i++){
    ...
    {
        buttons[i].classList.toggle('btnOn');
    });
}

//shading toggle
let shading = false;
const shaderButton = document.querySelector('.toggleShading');
shaderButton.addEventListener('click', () => {
    if (shading) {
        ...
        lighten = false;
        eraserButton.classList.remove('btnOn');
    }
    if(grab){
        grab = false;
        dropper.classList.remove('btnOn');
...
//lighten toggle
let lighten = false;
const lightenButton = document.querySelector('.toggleLighten');
lightenButton.addEventListener('click', () => {
    if (lighten){
        ...
        lighten = true;
        shading = false;
        shaderButton.classList.remove('.btnOn');
        rainbow = false;
        rainbowButton.classList.remove('.btnOn');
...
const dropper = document.querySelector('.colorGrabber');
let grab = false;
...
eraserButton.addEventListener('click', () => {
    if(eraser){
        ...
        rainbowButton.classList.remove('btnOn');
        lighten = false;
        lightenButton.classList.remove('btnOn');
    }
...
let rainbow = false;
const rainbowButton = document.querySelector('.toggleRainbow');
...
//turh the grid button back on if it is off
const gridButton = document.querySelector('#gridBtn');
if(gridButton.classList.contains('btnOn')){

} else {
    gridButton.classList.toggle('btnOn');
}
...
function fadeGrid(item){
      // if the cell hasnt been colored, set it to the background color (un marked cells are transparent)
...
const colorFillButton = document.querySelector('.colorFill');
let fill = false;
...
function colorFill(e) {
    if (fill) {
      ...
      colorFillButton.classList.remove('btnOn');
      fill = false;
    }
  }
